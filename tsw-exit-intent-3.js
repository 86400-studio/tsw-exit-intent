(function () {
  ...
  // Desktop: classic mouse-leave at top
  ...
  // Mobile: scroll-up near top + back-button, plus safety timer
  function initMobile() {
    var armed = false;
    var lastY =
      window.pageYOffset ||
    ...
    function onScroll() {
      ...
      // Once user has scrolled down a bit, we "arm" exit intent
      if (y > 250) {
        armed = true;
      }

      // If they scroll back near top after being armed, show popup
      if (armed && y < 40) {
    ...
    // Fallback timer: if they stay for 45s and still haven't seen it, show anyway
    setTimeout(function () {
      if (!hasShown()) {
        showPopup();
      }
    ...
    // Back-button handling (where history API works)
  ...
  onReady(function () {
    // Run on every page (including home) as long as Wix loads this script
    if (!hasShown()) {
      initDesktop();
      if (isMobile()) {
        initMobile();
